...
#if (KEY_REALNUM % PACK_SEND_COUNT_ONCE)
	#define PACK_IDX_MAX 			(KEY_REALNUM / PACK_SEND_COUNT_ONCE)
	#define PACK_SEND_COUNT_FINAL	(KEY_REALNUM % PACK_SEND_COUNT_ONCE)
#else
	#define PACK_IDX_MAX 			((KEY_REALNUM-1) / PACK_SEND_COUNT_ONCE)
	#define PACK_SEND_COUNT_FINAL	PACK_SEND_COUNT_ONCE
#endif

#define REL_TYPE_ID_USE	0x0A

typedef struct _PackStrcut
{
	u16 rawData;					//滤波值
	u16 baseData;					//基准值
	u16 positionData;				//位置值
	u16 noiseData;					//噪声值
}PackStrcut;
...
void REL_SenderLoop()
{
	...
	do
	{
		...
		packStrcut.baseData = _CMS_DispKeyDown[curIdx];
		packStrcut.positionData = _CMS_position;//
