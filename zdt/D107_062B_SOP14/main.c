...
#define		A_LED5_OUT					 TRISA	&= 0xFB
#define		A_LED4_OUT					 TRISA	&= 0xF7
#define		A_LED3_OUT					 TRISA	&= 0xEF
#define		A_LED2_OUT					 TRISA	&= 0xDF
...
void setBatStep()
{
	...
	else
	{
		...
		if(chrgFlag)
		{
			if(chrgFullFlag)
			{
				if(++count30s >= 3000 && showBatStep < 100)	//20s
				{	
					count30s = 0;
					showBatStep++;
				}
			...
			{
				if(++count30s >= 30000 && showBatStep < 100)	//20s
				{	
					count30s = 0;
					showBatStep++;
				}
