...
void keyCtr()
{
	char kclick = keyRead(0x10 & (~PORTB));
	if(kclick == 1)
	{
		if(workStep > 0)
		{
...
void checkOutA()
{
	...
	if (0xA5 == test_adc)				//测试完成，如因其他原因导致AD转换没有完成，则不处理
	{
        ...
        else if(outADValue > (maxtempV+100))
        {
        	if(++overCount > 5)
        	{
        		overCount = 5;
...
void Sleep_Mode()
{
	...
	TRISA = 0x03; 		//关闭所有输出，RA0口做唤醒输入
	WPUA  = 0B00000000;			//RA0 开上拉电阻
	PORTA = 0x3C;
	TRISB = 0x14;			//关闭所有输出，RB3口做唤醒输入
	PORTB = 0x03;
	WPUB  = 0x10;			//RB2 开上拉电阻
