...
#define		A_LED5_OUT					 TRISA	&= 0xEF
#define		A_LED4_OUT					 TRISA	&= 0xF7
#define		A_LED3_OUT					 TRISB	&= 0xDF
#define		A_LED2_OUT					 TRISB	&= 0xFE
...
{
	maxWorkTime = 450;
	if(workTime > 390 || workTime % 100 > 90)
	{
		pwmStop();
	}
	else if(motorPwmFlag == 0)
	{
		pwmInit();
		PWMD23H = 0;
		PWMD3L = 172;
...
void ctr9()
{
	...
	if(workTime % 2 == 0)
	{
		if(workTime < 260 && curDuty < 199)
		{
			curDuty++;
		}
		else if(workTime < 340)
		{
			curDuty = 199;
		}
		else if(workTime >= 340 && curDuty > 70)
		{
			curDuty--;
		}
		PWMD23H = 0;
...
void pwmInit()
{
		...
		PWMTH = 0X00;			//周期选择为0FF，则周期为（255+1）*1/8M，周期的时钟分频在PWMCON0选择
		PWMTL = 200;			//PWM 周期=[PWMT+1]*Tosc*(CLKDIV 分频值)
		
		
		PWMD23H = 0;			//配置PWM1,PWM2的占空比，该值不能超过周期，否者为100%输出
		PWMD3L = 80;			//互补模式，PWM1的占空比与PWM0互补，和设置值无关
		PWM23DT = 0X00;			//死区设为2uS，计算为（3+1）*1/8M*4

		PWMCON0 = 0X48;			//打开PWM1 分频比为8
		PWMCON2 =0X00;			//PWM4输出反向
}
...
void Sleep_Mode()
{
	...
	IOCA = 0x20;			//允许RA1的IO电平变化中断
	IOCB = 0x04;			//允许RB3的IO口电平变化中断
