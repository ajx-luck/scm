/**********************************************************
C212不加力的做一款1-5档打打停停的（按档位速度和力气慢慢增快），六档渐进模式（从小到最大）
**********************************************************/
...
const static unsigned char startTimes[]={160,100,60,43,31};
const static unsigned char stopTimes[]={70,49,27,19,18,12};
...
void workCtr()
{
	...
	if(workStep > 0)
	{
		if(workStep == 6)
		{
			...
			if(modelTime % 5 == 0)
			{
				if(modelTime < 300 && maxDuty < 99)
				{
					maxDuty++;
				}
				if(modelTime > 300 && maxDuty > 39)
				{
					maxDuty--;
				}
			}
		...
		{
			u16t tempStartTime = startTimes[workStep - 1];
			u16t tempStopTime = stopTimes[workStep - 1];
