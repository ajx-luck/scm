...
const unsigned char tmp1 @ 0xf00 = 0xaa;	//定义滚码地址
const unsigned char tmp2 @ 0xf01 = 0xaa;
const unsigned char tmp3 @ 0xf02 = 0xaa;
const unsigned char tmp4 @ 0xf03 = 0xaa;
...
void main()
{
	...
	***************************************************************************/
	...
	while(1)
	{
		...
		{
			Memory_Write(datanum,0x55aa);			//调用写函数
			Memory_Read(datanum);				//调用读函数
			if(0x55aa == read_data)
			{
				data++;
			}
		...
		{
			unsigned int read_addr = 0x0f00;
			read_addr = read_addr + datanum;		//赋值滚码所在地址
			Program_Read(read_addr);				//调用读函数
